...
static status_t cmd_file_filter(const std::vector<std::string> &args)
{
    if (args.size() < 4)
        return STATUS_INVALID;
    ...
    std::ifstream fin(args[2], std::ios_base::binary);
    mf_char ch;
    
    while ((ch = mf_getchar(fin)) != 0)
    {
        allowed.insert(ch);
    }
    
    std::string src = args.at(1);
    std::string dst = args.at(3);
...
static status_t cmd_rlefont_export(const std::vector<std::string> &args)
{
    ...
    std::string src = args.at(1);
    std::string dst = (args.size() == 2) ? strip_extension(src) + ".cpp" : args.at(2);
    std::unique_ptr<DataFile> f = load_dat(src);
...
static const char *usage_msg =
    ...
    "   file_filter <datfile> <charset_file> <output>  Remove everything except specified characters.\n"
